...
func TestAxpyUnitary(t *testing.T) {
	...
	for i, tt := range axpyTests {
		for _, align := range align2 {
			prefix := fmt.Sprintf("Test %v (x:%v y:%v)", i, align.x, align.y)
			xgLn, ygLn := 4+align.x, 4+align.y
			xg, yg := guardVector(tt.x, xGdVal, xgLn), guardVector(tt.y, yGdVal, ygLn)
			x, y := xg[xgLn:len(xg)-xgLn], yg[ygLn:len(yg)-ygLn]
			AxpyUnitary(tt.alpha, x, y)
			for i := range tt.want {
				if !same(y[i], tt.want[i]) {
					t.Errorf(msgVal, prefix, i, y[i], tt.want[i])
				}
			}
...
func TestAxpyUnitaryTo(t *testing.T) {
	var dstGdVal, xGdVal, yGdVal float64 = 1, -1, 0.5
	for i, tt := range axpyTests {
		for _, align := range align3 {
			prefix := fmt.Sprintf("Test %v (x:%v y:%v dst:%v)", i, align.x, align.y, align.z)

			dgLn, xgLn, ygLn := 4+align.z, 4+align.x, 4+align.y
			dstOrig := make([]float64, len(tt.x))
			xg, yg := guardVector(tt.x, xGdVal, xgLn), guardVector(tt.y, yGdVal, ygLn)
			dstg := guardVector(dstOrig, dstGdVal, dgLn)
			x, y := xg[xgLn:len(xg)-xgLn], yg[ygLn:len(yg)-ygLn]
			dst := dstg[dgLn : len(dstg)-dgLn]

			AxpyUnitaryTo(dst, tt.alpha, x, y)
			for i := range tt.want {
				if !same(dst[i], tt.want[i]) {
					t.Errorf(msgVal, prefix, i, dst[i], tt.want[i])
				}
			}
			if !isValidGuard(xg, xGdVal, xgLn) {
				t.Errorf(msgGuard, prefix, "x", xg[:xgLn], xg[len(xg)-xgLn:])
...
func TestAxpyInc(t *testing.T) {
	...
	for i, test := range axpyTests {
		n := len(test.x)
		for _, inc := range genInc([]int{-7, -4, -3, -2, -1, 1, 2, 3, 4, 7}) {
			var ix, iy int
			if inc.x < 0 {
				ix = (-n + 1) * inc.x
			}
...
func TestAxpyIncTo(t *testing.T) {
	...
	for i, tt := range axpyTests {
		n := len(tt.x)
		for _, inc := range genIncTo([]int{-7, -4, -3, -2, -1, 1, 2, 3, 4, 7}) {
			...
			dstOrig := make([]float64, len(tt.want))
			xg := guardIncVector(tt.x, xGdVal, inc.x, gdLn)
			yg := guardIncVector(tt.y, yGdVal, inc.y, gdLn)
			dstg := guardIncVector(dstOrig, dstGdVal, inc.dst, gdLn)
			x, y := xg[gdLn:len(xg)-gdLn], yg[gdLn:len(yg)-gdLn]
			dst := dstg[gdLn : len(dstg)-gdLn]

			AxpyIncTo(dst, uintptr(inc.dst), idst,
				tt.alpha, x, y, uintptr(n),
			...
			if !equalStrided(tt.x, x, inc.x) {
				t.Errorf("%v: modified read-only x argument", prefix)
			}
			if !equalStrided(tt.y, y, inc.y) {
				t.Errorf("%v: modified read-only y argument", prefix)
			}
		}
	}
