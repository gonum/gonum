package concrete_test

import (
	"testing"

	gr "github.com/gonum/graph"
	"github.com/gonum/graph/concrete"
)

var _ gr.Graph = &concrete.GonumGraph{}
var _ gr.DirectedGraph = &concrete.GonumGraph{}
var _ gr.MutableGraph = &concrete.GonumGraph{}
var _ gr.EdgeListGraph = &concrete.GonumGraph{}

// exercise methods for 100% coverage.
func TestGonumGraphCoverage(t *testing.T) {
	var n1 gr.Node = concrete.Node(1)
	var e gr.Edge = concrete.Edge{n1, n1}
	e.Head()
	e.Tail()
	concrete.NewPreAllocatedGonumGraph(true, 1)
	dg := concrete.NewGonumGraph(true)                     // directed
	var dm gr.MutableGraph = dg                            // directed mutable
	var um gr.MutableGraph = concrete.NewGonumGraph(false) // undirected
	dm.AddNode(n1, nil)
	n0 := dm.NewNode(nil) // fill in hole, node 0
	dm.NewNode(nil)       // node 2
	dm.AddNode(n1, nil)   // no op
	um.AddNode(n1, nil)   // undirected
	n3 := concrete.Node(3)
	n4 := concrete.Node(4)
	dm.AddNode(n3, []gr.Node{n4}) // both node and successor are new
	um.AddNode(n0, []gr.Node{n1}) // new undirected edge
	dm.AddEdge(e)
	n5 := concrete.Node(5)
	dm.AddEdge(concrete.Edge{n5, n1}) // head not in graph
	dm.AddEdge(concrete.Edge{n1, n5}) // tail not in graph
	um.AddEdge(concrete.Edge{n1, n0}) // undirected
	n6 := concrete.Node(6)
	dm.SetEdgeCost(concrete.Edge{n6, n1}, 0) // n6 not in graph
	dm.SetEdgeCost(concrete.Edge{n1, n6}, 0)
	um.SetEdgeCost(concrete.Edge{n0, n1}, 0) // undirected
	dm.AddEdge(concrete.Edge{n5, n0})
	dm.RemoveNode(n5)
	dm.RemoveNode(n5)
	dm.RemoveEdge(concrete.Edge{n6, n0})
	dm.RemoveEdge(concrete.Edge{n0, n6})
	um.RemoveEdge(concrete.Edge{n0, n1})
	dm.SetDirected(false)
	dm.EmptyGraph()
	dm.EmptyGraph()
	dm.SetDirected(true)
	d := dm.(gr.DirectedGraph)
	d.Successors(n0)
	dm.AddNode(n0, []gr.Node{n1})
	d.Successors(n0)
	d.IsSuccessor(n3, n1)
	d.IsSuccessor(n1, n3)
	d.Predecessors(n3)
	d.Predecessors(n1)
	d.IsPredecessor(n3, n1)
	d.IsPredecessor(n1, n3)
	u := um.(gr.Graph)
	u.Neighbors(n3)
	um.AddEdge(concrete.Edge{n0, n1})
	u.Neighbors(n1)
	dg.Neighbors(n1) // directed graph neighbors
	d.IsNeighbor(n3, n1)
	d.IsNeighbor(n1, n3)
	d.NodeExists(n1)
	d.Degree(n3)
	um.AddEdge(concrete.Edge{n0, n0})
	d.Degree(n0)
	u.Degree(n0)
	u.NodeList()
	u.(gr.EdgeLister).EdgeList()
	dg.IsDirected()
	dm.Cost(n0, n1)
	dm.Cost(n6, n1)
}
