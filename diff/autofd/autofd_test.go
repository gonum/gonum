...
`,
	...
	return dual.Add(dual.Add(dual.Mul(dual.Number{Real:2}, dual.Number{Real:x, Emag:1}), dual.Mul(dual.Mul(dual.Number{Real:3}, dual.Number{Real:x, Emag:1}), dual.Number{Real:x, Emag:1})), dual.Mul(dual.Number{Real:4}, dual.Pow(dual.Number{Real:x, Emag:1}, dual.Number{Real:3}))).Emag
}
`,
		},
		{
...
`,
	...
	return dual.Mul(dual.Number{Real:y, Emag:1}, dual.Number{Real:y, Emag:1}).Emag
}
`,
		},
		{
	...
	return dual.Mul(dual.Mul(dual.Number{Real:2}, dual.Number{Real:x, Emag:1}), dual.Number{Real:x, Emag:1}).Emag
}
`,
		},
		{
	...
	return dual.Mul(dual.Number{Real:2}, dual.Inv((dual.Mul(dual.Number{Real:x, Emag:1}, dual.Number{Real:x, Emag:1})))).Emag
}
`,
		},
		{
	...
	return dual.Mul(dual.Number{Real:2}, dual.Inv((dual.Mul(dual.Number{Real:x, Emag:1}, dual.Mul(dual.Number{Real:1, Emag:1}, dual.Number{Real:x, Emag:1}))))).Emag
}
`,
		},
		{
	...
	return dual.Sub(dual.Add(dual.Number{Real:2}, dual.Number{Real:x, Emag:1}), dual.Number{Real:x, Emag:1}).Emag
}
`,
		},
		{
	...
	return dual.Cos(dual.Mul(dual.Mul(dual.Number{Real:2}, dual.Number{Real: math.Pi}), dual.Number{Real:x, Emag:1})).Emag
}
`,
		},
		{
	...
	return dual.Mul(dual.Exp(dual.Number{Real:x, Emag:1}), dual.Inv(dual.Sqrt(dual.Add(dual.Pow(dual.Sin(dual.Number{Real:x, Emag:1}), dual.Number{Real:3}), dual.Pow(dual.Cos(dual.Number{Real:x, Emag:1}), dual.Number{Real:3}))))).Emag
}
`,
		},
		{
		...
		},
		{
			...
			err:  fmt.Errorf(`could not create derivative generator: could not lookup "F1xxx" in package "gonum.org/v1/gonum/diff/autofd/internal/testfunc"`),
		},
		{
			name: autofd.Func{Path: "gonum.org/v1/gonum/diff/autofd/internal/testfunc", Name: "Fxxx.F"},
			err:  fmt.Errorf(`could not create derivative generator: could not lookup "Fxxx" in package "gonum.org/v1/gonum/diff/autofd/internal/testfunc"`),
		},
		{
		...
		name := fmt.Sprintf("%s.%s", tc.name.Path, tc.name.Name)
		if tc.name.Deriv != "" {
			name += "-" + tc.name.Deriv
		}
		t.Run(name, func(t *testing.T) {
			buf := new(strings.Builder)
			err := autofd.Derivative(buf, tc.name)
			switch {
