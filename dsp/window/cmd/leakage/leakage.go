...
func main() {
	name := flag.String("window", "", "specify a built-in window name")
	...
	csv := flag.String("csv", "", "specify an input file of frequency amplitudes (dB transformed)")
	out := flag.String("o", "", "specify output file for plot")
	...
	if win == nil && *csv == "" {
		fmt.Fprintf(os.Stderr, "missing function name or csv input")
		flag.Usage()
		os.Exit(2)
	}
	if *csv == "" && !win.ok(*param) {
		fmt.Fprintf(os.Stderr, "missing parameter")
		flag.Usage()
		os.Exit(2)
	}
	if *out == "" {
