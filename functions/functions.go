...
func (Beale) Hess(hess *mat64.SymDense, x []float64) {
	if len(x) != 2 {
		panic("dimension of the problem must be 2")
	}
...
func (BrownBadlyScaled) Hess(hess *mat64.SymDense, x []float64) {
	if len(x) != 2 {
		panic("dimension of the problem must be 2")
	}
	if len(x) != hess.Symmetric() {
...
func (BrownAndDennis) Hess(hess *mat64.SymDense, x []float64) {
	if len(x) != 4 {
		panic("dimension of the problem must be 4")
	}
	if len(x) != hess.Symmetric() {
...
func (PowellBadlyScaled) Hess(hess *mat64.SymDense, x []float64) {
	if len(x) != 2 {
		panic("dimension of the problem must be 2")
	}
	if len(x) != hess.Symmetric() {
...
func (Watson) Hess(hess *mat64.SymDense, x []float64) {
	dim := len(x)
...
func (Wood) Hess(hess *mat64.SymDense, x []float64) {
	if len(x) != 4 {
		panic("dimension of the problem must be 4")
	}
	if len(x) != hess.Symmetric() {
