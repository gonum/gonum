...
// given nodes.
func colorsOf(nodes graph.Nodes, coloring map[int64]int) set.Set[int] {
	c := make(set.Set[int], nodes.Len())
	for nodes.Next() {
...
func newSaturationDegree(it graph.Nodes, g graph.Undirected, colors map[int64]int) saturationDegree {
	...
	sd := saturationDegree{
		...
		adjColors: make([]set.Set[int], len(nodes)),
		g:         g,
		colors:    colors,
	}
	for i, u := range nodes {
		sd.degrees[i] = g.From(u.ID()).Len()
		sd.adjColors[i] = make(set.Set[int])
		sd.indexOf[u.ID()] = i
	}
...
func (sd *saturationDegree) same(vi, vj set.Set[int]) int {
	valid := make(set.Set[int])
