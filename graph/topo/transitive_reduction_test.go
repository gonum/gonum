...
func TestTransitiveReduce2_ChainWithShortcuts(t *testing.T) {
	orig := makeChainWithShortcuts(8)
	before := cloneDirected(orig)
	after := cloneDirected(orig)
...
func TestTransitiveReduce2_Diamond(t *testing.T) {
	orig := makeDiamondWithShortcut()
	before := cloneDirected(orig)
	after := cloneDirected(orig)

	if err := TransitiveReduce2(after); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestTransitiveReduce2_AlreadyReduced(t *testing.T) {
	orig := makeAlreadyReduced()
	before := cloneDirected(orig)
	after := cloneDirected(orig)
...
func TestTransitiveReduce2_Disconnected(t *testing.T) {
	orig := makeDisconnected()
	before := cloneDirected(orig)
	after := cloneDirected(orig)
...
func TestTransitiveReduce2_EmptyGraph(t *testing.T) {
	g := simple.NewDirectedGraph()
	before := cloneDirected(g)
	after := cloneDirected(g)
...
func TestTransitiveReduce2_SingleNodeNoEdges(t *testing.T) {
	g := simple.NewDirectedGraph()
	g.AddNode(simple.Node(1))
...
func TestTransitiveReduce2_TwoNodesSingleEdge(t *testing.T) {
	g := simple.NewDirectedGraph()
	g.AddNode(simple.Node(1))
	g.AddNode(simple.Node(2))
	g.SetEdge(simple.Edge{F: simple.Node(1), T: simple.Node(2)})
...
func TestTransitiveReduce2_CycleReturnsError(t *testing.T) {
	g := simple.NewDirectedGraph()
	for _, id := range []int64{1, 2, 3} {
		g.AddNode(simple.Node(id))
	}
...
func TestTransitiveReduce2_StarNoTransitiveEdges(t *testing.T) {
	// 0 -> {1,2,3,4} and no other edges => nothing is redundant.
	g := simple.NewDirectedGraph()
	for i := int64(0); i <= 4; i++ {
		g.AddNode(simple.Node(i))
...
func TestTransitiveReduce2_LayeredDAGManyRedundantEdges(t *testing.T) {
	// Layers:
	// 0 -> (1,2,3)
	// (1,2,3) -> (4,5,6)
	// (4,5,6) -> 7
...
func TestTransitiveReduce2_RandomSmallDAG(t *testing.T) {
	// Same graph as above, but for TransitiveReduce2.
	const n = 12
	g := simple.NewDirectedGraph()
	for i := 0; i < n; i++ {
		g.AddNode(simple.Node(int64(i)))
