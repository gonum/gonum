...
//
// Left eigenvectors will be computed only if jobvl == ComputeLeftEV,
// otherwise jobvl must be None. Right eigenvectors will be computed
// only if jobvr == ComputeRightEV, otherwise jobvr must be None.
...
// converged.
func (impl Implementation) Dgeev(jobvl LeftEVJob, jobvr RightEVJob, n int, a []float64, lda int, wr, wi []float64, vl []float64, ldvl int, vr []float64, ldvr int, work []float64, lwork int) (first int) {
	var wantvl bool
	switch jobvl {
	default:
	...
	if wantvl || wantvr {
		...
		impl.Dhseqr(EigenvaluesAndSchur, OriginalEV, n, 0, n-1,
		...
		impl.Dtrevc3(side, AllEVMulQ, nil, n, nil, 1, nil, 1, nil, 1,
			n, work, -1)
		maxwrk = max(maxwrk, n+int(work[0]))
		maxwrk = max(maxwrk, 4*n)
	} else {
		impl.Dhseqr(EigenvaluesOnly, None, n, 0, n-1,
			nil, 1, nil, nil, nil, 1, work, -1)
		maxwrk = max(maxwrk, max(n+1, n+int(work[0])))
	}
	...
	anrm := impl.Dlange(MaxAbs, n, n, a, lda, nil)
	var scalea bool
	var cscale float64
	...
	if scalea {
		impl.Dlascl(General, 0, 0, anrm, cscale, n, n, a, lda)
	}

	// Balance the matrix.
	workbal := work[:n]
	ilo, ihi := impl.Dgebal(PermuteScale, n, a, lda, workbal)
	...
	if wantvl {
		...
		iwrk = n
		first = impl.Dhseqr(EigenvaluesAndSchur, OriginalEV, n, ilo, ihi,
		...
		iwrk = n
		first = impl.Dhseqr(EigenvaluesOnly, None, n, ilo, ihi,
	...
	if first > 0 {
		if scalea {
			// Undo scaling.
			impl.Dlascl(General, 0, 0, cscale, anrm, n-first, 1, wr[first:], 1)
			impl.Dlascl(General, 0, 0, cscale, anrm, n-first, 1, wi[first:], 1)
			impl.Dlascl(General, 0, 0, cscale, anrm, ilo, 1, wr, 1)
			impl.Dlascl(General, 0, 0, cscale, anrm, ilo, 1, wi, 1)
		}
		work[0] = float64(maxwrk)
		return first
	}
	...
	if wantvl || wantvr {
		...
		impl.Dtrevc3(side, AllEVMulQ, nil, n,
			a, lda, vl, ldvl, vr, ldvr, n, work[iwrk:], lwork-iwrk)
	}
	bi := blas64.Implementation()
	if wantvl {
		// Undo balancing of left eigenvectors.
		impl.Dgebak(PermuteScale, LeftEV, n, ilo, ihi, workbal, n, vl, ldvl)
	...
	if wantvr {
		...
		impl.Dgebak(PermuteScale, RightEV, n, ilo, ihi, workbal, n, vr, ldvr)
