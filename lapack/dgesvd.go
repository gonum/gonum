...
// is as follows
//  jobU == SVDAll       All m columns of U are returned in u
//  jobU == SVDInPlace   The first min(m,n) columns are returned in u
//  jobU == SVDOverwrite The first min(m,n) columns of U are written into a
//  jobU == SVDNone      The columns of U are not computed.
// The behavior is the same for jobVT and the rows of V^T. At most one of jobU
// and jobVT can equal SVDOverwrite, and Dgesvd will panic otherwise.
//
...
// jobU == SVDAll, u is of size m×m. If jobU == SVDInPlace u is
// of size m×min(m,n). If jobU == SVDOverwrite or SVDNone, u is
// not used.
//
// vt contains the left singular vectors on exit, stored row-wise. If
// jobV == SVDAll, vt is of size n×m. If jobVT == SVDInPlace vt is
// of size min(m,n)×n. If jobVT == SVDOverwrite or SVDNone, vt is
// not used.
//
...
func (impl Implementation) Dgesvd(jobU, jobVT SVDJob, m, n int, a []float64, lda int, s, u []float64, ldu int, vt []float64, ldvt int, work []float64, lwork int) (ok bool) {
	minmn := min(m, n)
	checkMatrix(m, n, a, lda)
	if jobU == SVDAll {
		checkMatrix(m, m, u, ldu)
	} else if jobU == SVDInPlace {
		checkMatrix(m, minmn, u, ldu)
	}
	if jobVT == SVDAll {
		checkMatrix(n, n, vt, ldvt)
	} else if jobVT == SVDInPlace {
		checkMatrix(minmn, n, vt, ldvt)
	}
	if jobU == SVDOverwrite && jobVT == SVDOverwrite {
		panic("lapack: both jobU and jobVT are SVDOverwrite")
	}
	if len(s) < minmn {
		panic(badS)
	}
	...
	wantua := jobU == SVDAll
	wantus := jobU == SVDInPlace
	wantuas := wantua || wantus
	wantuo := jobU == SVDOverwrite
	wantun := jobU == None

	wantva := jobVT == SVDAll
	wantvs := jobVT == SVDInPlace
	wantvas := wantva || wantvs
	wantvo := jobVT == SVDOverwrite
	wantvn := jobVT == None
	...
	if m >= n {
		...
		impl.Dorgbr(ApplyP, n, n, n, a, lda, nil, work, -1)
		lwork_dorgbr_p := int(work[0])
		impl.Dorgbr(ApplyQ, n, n, n, a, lda, nil, work, -1)
		...
		if m >= mnthr {
			...
			if wantus || wantuo {
				impl.Dorgbr(ApplyQ, m, n, n, a, lda, nil, work, -1)
				lwork_dorgbr_q = int(work[0])
				maxwrk = max(maxwrk, 3*n+lwork_dorgbr_q)
			}
			if wantua {
				impl.Dorgbr(ApplyQ, m, m, n, a, lda, nil, work, -1)
				lwork_dorgbr_q := int(work[0])
				maxwrk = max(maxwrk, 3*n+lwork_dorgbr_q)
			}
			if !wantvn {
		...
		lwork_dgebrd := int(work[0])
		impl.Dorgbr(ApplyP, m, m, m, a, n, nil, work, -1)
		lwork_dorgbr_p := int(work[0])
		impl.Dorgbr(ApplyQ, m, m, m, a, n, nil, work, -1)
		lwork_dorgbr_q := int(work[0])
		if n >= mnthr {
			// n >> m
			if wantvn {
			...
			if wantvs || wantvo {
				impl.Dorgbr(ApplyP, m, n, m, a, n, nil, work, -1)
				lwork_dorgbr_p = int(work[0])
				maxwrk = max(maxwrk, 3*m+lwork_dorgbr_p)
			}
			if wantva {
				impl.Dorgbr(ApplyP, n, n, m, a, n, nil, work, -1)
				lwork_dorgbr_p = int(work[0])
				maxwrk = max(maxwrk, 3*m+lwork_dorgbr_p)
			}
			if !wantun {
	...
	anrm := impl.Dlange(MaxAbs, m, n, a, lda, nil)
	var iscl bool
	if anrm > 0 && anrm < smlnum {
		iscl = true
		impl.Dlascl(General, 0, 0, anrm, smlnum, m, n, a, lda)
	} else if anrm > bignum {
		iscl = true
		impl.Dlascl(General, 0, 0, anrm, bignum, m, n, a, lda)
	}
	...
	if m >= n {
		...
		if m >= mnthr {
			...
			if wantun {
				...
				if wantvo || wantvas {
					// Generate P^T.
					impl.Dorgbr(ApplyP, n, n, n, a, lda, work[itaup:],
				...
				if wantvn {
					...
					if lwork >= n*n+max(4*n, bdspac) {
						...
						impl.Dorgbr(ApplyQ, n, n, n, work[ir:], ldworkr,
						...
						impl.Dormbr(ApplyQ, blas.Right, blas.NoTrans, m, n, n,
					...
					if lwork >= n*n+max(4*n, bdspac) {
						...
						impl.Dorgbr(ApplyQ, n, n, n, work[iu:], ldworku,
							work[itauq:], work[iwork:], lwork-iwork)

						// Generate right bidiagonalizing vectors in VT.
						impl.Dorgbr(ApplyP, n, n, n, vt, ldvt,
							work[itaup:], work[iwork:], lwork-iwork)
						iwork = ie + n
				...
				if wantvn {
					...
					if lwork >= n*n+max(max(n+m, 4*n), bdspac) {
						...
						/*
							...
							// Multiply Q in U by left bidiagonalizing vectors in VT.
							impl.Dormbr(ApplyQ, blas.Right, blas.NoTrans,
			...
			if wantuas {
				...
				impl.Dorgbr(ApplyQ, m, ncu, n, u, ldu, work[itauq:], work[iwork:], lwork-iwork)
			}
			if wantvas {
				// Right singular vectors are desired in VT. Copy result to VT and
				// generate left biadiagonalizing vectors in VT.
				impl.Dlacpy(blas.Upper, n, n, a, lda, vt, ldvt)
				impl.Dorgbr(ApplyP, n, n, n, vt, ldvt, work[itaup:], work[iwork:], lwork-iwork)
			}
			if wantuo {
				panic(noSVDO)
			}
		...
		if n >= mnthr {
			...
			if wantvn {
				...
				if wantuo || wantuas {
					impl.Dorgbr(ApplyQ, m, m, m, a, lda,
						work[itauq:], work[iwork:], lwork-iwork)
				}
				iwork = ie + m
				nru := 0
				...
				if wantun {
					...
					if lwork >= m*m+max(4*m, bdspac) {
						...
						impl.Dorgbr(ApplyP, m, m, m, work[ir:], ldworkr,
							work[itaup:], work[iwork:], lwork-iwork)
						iwork = ie + m
						...
						impl.Dormbr(ApplyP, blas.Left, blas.Trans, m, n, m,
					...
					if lwork >= m*m+max(4*m, bdspac) {
						...
						impl.Dorgbr(ApplyP, m, m, m, work[iu:], ldworku,
							work[itaup:], work[iwork:], lwork-iwork)

						// Generate left bidiagonalizing vectors in U.
						impl.Dorgbr(ApplyQ, m, m, m, u, ldu, work[itauq:], work[iwork:], lwork-iwork)
						iwork = ie + m
			...
			if wantuas {
				...
				// generate left bidiagonalizing vectors in U.
				impl.Dlacpy(blas.Lower, m, m, a, lda, u, ldu)
				impl.Dorgbr(ApplyQ, m, m, n, u, ldu, work[itauq:], work[iwork:], lwork-iwork)
			}
			if wantvas {
				...
				impl.Dorgbr(ApplyP, nrvt, n, m, vt, ldvt, work[itaup:], work[iwork:], lwork-iwork)
			}
			if wantuo {
				panic(noSVDO)
			}
	...
	if iscl {
		if anrm > bignum {
			impl.Dlascl(General, 0, 0, bignum, anrm, minmn, 1, s, minmn)
		}
		if !ok && anrm > bignum {
			impl.Dlascl(General, 0, 0, bignum, anrm, minmn-1, 1, work[minmn:], minmn)
		}
		if anrm < smlnum {
			impl.Dlascl(General, 0, 0, smlnum, anrm, minmn, 1, s, minmn)
		}
		if !ok && anrm < smlnum {
			impl.Dlascl(General, 0, 0, smlnum, anrm, minmn-1, 1, work[minmn:], minmn)
		}
	}
	work[0] = float64(maxwrk)
	return ok
