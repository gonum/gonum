...
func (impl Implementation) Dggsvd3(jobU, jobV, jobQ GSVDJob, m, n, p int, a []float64, lda int, b []float64, ldb int, alpha, beta, u []float64, ldu int, v []float64, ldv int, q []float64, ldq int, work []float64, lwork int, iwork []int) (k, l int, ok bool) {
	...
	} else if jobU != GSVDNone {
		panic(badGSVDJob + "U")
	}
	wantv := jobV == GSVDV
	if wantv {
		checkMatrix(p, p, v, ldv)
	} else if jobV != GSVDNone {
		panic(badGSVDJob + "V")
	}
	wantq := jobQ == GSVDQ
	if wantq {
		checkMatrix(n, n, q, ldq)
	} else if jobQ != GSVDNone {
		panic(badGSVDJob + "Q")
	}
	...
	anorm := impl.Dlange(NormFrob, m, n, a, lda, nil)
	bnorm := impl.Dlange(NormFrob, p, n, b, ldb, nil)
