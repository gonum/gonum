...
//  A = Q H Q^T = (QZ) T (QZ)^T.
//
// If job == EigenvaluesOnly, only the eigenvalues will be computed.
// If job == EigenvaluesAndSchur, the eigenvalues and the Schur form T will
// be computed.
// For other values of job Dhseqr will panic.
//
// If compz == None, no Schur vectors will be computed and Z will not be
// referenced.
// If compz == HessEV, on return Z will contain the matrix of Schur
// vectors of H.
// If compz == OriginalEV, on entry z is assumed to contain the orthogonal
...
// and wi[i+1] < 0.
//
// If unconverged == 0 and job == EigenvaluesAndSchur, on return H will
...
//  wi[i+1] = -wi[i].
//
// If unconverged == 0 and job == EigenvaluesOnly, the contents of h
...
//
// If unconverged > 0 and compz == OriginalEV, then on return
//  (final Z) = (initial Z) U,
// where U is the orthogonal matrix in (*) regardless of the value of job.
//
// If unconverged > 0 and compz == HessEV, then on return
//  (final Z) = U,
// where U is the orthogonal matrix in (*) regardless of the value of job.
//
// References:
...
func (impl Implementation) Dhseqr(job EVJob, compz EVComp, n, ilo, ihi int, h []float64, ldh int, wr, wi []float64, z []float64, ldz int, work []float64, lwork int) (unconverged int) {
	var wantt bool
	switch job {
	default:
		panic(badEVJob)
	case EigenvaluesOnly:
	case EigenvaluesAndSchur:
		wantt = true
	}
	var wantz bool
	switch compz {
