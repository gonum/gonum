...
// in t.
//  H = I - V * T * V^T  if store == ColumnWise
//  H = I - V^T * T * V  if store == RowWise
...
func (Implementation) Dlarft(direct Direct, store StoreV, n, k int,
	...
	if store != RowWise && store != ColumnWise {
		panic(badStore)
	}
	if len(tau) < k {
		panic(badTau)
