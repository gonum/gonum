...
// by a depending on the value of side -- A = P * A if side == Left,
// A = A * P^T if side == Right.
//
...
//  R(k) = [ c[k] s[k]]
//         [-s[k] s[k]]
// If direct == Forward, the rotation matrices are applied as
// P = P(z-1) * ... * P(2) * P(1), while if direct == Backward they are
// applied as P = P(1) * P(2) * ... * P(n).
//
// pivot defines the mapping of the elements in R(k) to P(k).
// If pivot == Variable, the rotation is performed for the (k, k+1) plane.
//  P(k) = [1                    ]
//         [    ...              ]
//         [     1               ]
//         [       c[k] s[k]     ]
...
//         [                ...  ]
//         [                    1]
// if pivot == Top, the rotation is performed for the (1, k+1) plane,
//  P(k) = [c[k]        s[k]     ]
//         [    1                ]
//         [     ...             ]
//         [         1           ]
...
//         [                ...  ]
//         [                    1]
// and if pivot == Bottom, the rotation is performed for the (k, z) plane.
//  P(k) = [1                    ]
//         [  ...                ]
//         [      1              ]
//         [        c[k]     s[k]]
...
func (impl Implementation) Dlasr(side blas.Side, pivot Pivot, direct Direct, m, n int, c, s, a []float64, lda int) {
