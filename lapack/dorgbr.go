...
// Q and P^T.
//
// If vect == ApplyQ, then a is assumed to have been an m×k matrix and
...
//
// If vect == ApplyP, then A is assumed to have been a k×n matrix, and
...
func (impl Implementation) Dorgbr(vect DecompUpdate, m, n, k int, a []float64, lda int, tau, work []float64, lwork int) {
	mn := min(m, n)
	wantq := vect == ApplyQ
	if wantq {
