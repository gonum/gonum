...
//  Q * C   if vect == ApplyQ, side == blas.Left, and trans == blas.NoTrans
//  C * Q   if vect == ApplyQ, side == blas.Right, and trans == blas.NoTrans
//  Q^T * C if vect == ApplyQ, side == blas.Left, and trans == blas.Trans
//  C * Q^T if vect == ApplyQ, side == blas.Right, and trans == blas.Trans
//
//  P * C   if vect == ApplyP, side == blas.Left, and trans == blas.NoTrans
//  C * P   if vect == ApplyP, side == blas.Right, and trans == blas.NoTrans
//  P^T * C if vect == ApplyP, side == blas.Left, and trans == blas.Trans
//  C * P^T if vect == ApplyP, side == blas.Right, and trans == blas.Trans
...
//
// If vect == ApplyQ, A is assumed to have been an nq×k matrix, while if
// vect == ApplyP, A is assumed to have been a k×nq matrix. nq = m if
...
func (impl Implementation) Dormbr(vect DecompUpdate, side blas.Side, trans blas.Transpose, m, n, k int, a []float64, lda int, tau, c []float64, ldc int, work []float64, lwork int) {
