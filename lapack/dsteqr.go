...
// tridiagonal form if compz == OriginalEV. On exit, if
// compz == OriginalEV, z contains the orthonormal eigenvectors of the
// original symmetric matrix, and if compz == TridiagEV, z contains the
...
func (impl Implementation) Dsteqr(compz EVComp, n int, d, e, z []float64, ldz int, work []float64) (ok bool) {
	if n < 0 {
		panic(nLT0)
	}
	if len(d) < n {
	...
	if compz != None && compz != TridiagEV && compz != OriginalEV {
		panic(badEVComp)
	}
	if compz != None {
	...
	for {
		if l1 > n-1 {
			...
			if icompz == 0 {
				impl.Dlasrt(SortIncreasing, n, d)
			} else {
		...
		anorm := impl.Dlanst(MaxAbs, lend-l+1, d[l:], e[l:])
		switch {
		case anorm == 0:
			continue
		case anorm > ssfmax:
			...
			impl.Dlascl(General, 0, 0, anorm, ssfmax, lend-l+1, 1, d[l:], 1)
			impl.Dlascl(General, 0, 0, anorm, ssfmax, lend-l, 1, e[l:], 1)
		case anorm < ssfmin:
			iscale = up
			impl.Dlascl(General, 0, 0, anorm, ssfmin, lend-l+1, 1, d[l:], 1)
			impl.Dlascl(General, 0, 0, anorm, ssfmin, lend-l, 1, e[l:], 1)
		}
			...
			for {
				...
				if m == l+1 {
					if icompz > 0 {
						...
						impl.Dlasr(blas.Right, Variable, Backward,
			...
			for {
				...
				if m == l-1 {
					if icompz > 0 {
						...
						impl.Dlasr(blas.Right, Variable, Forward,
		...
		case down:
			// Pretend that d and e are matrices with 1 column.
			impl.Dlascl(General, 0, 0, ssfmax, anorm, lendsv-lsv+1, 1, d[lsv:], 1)
			impl.Dlascl(General, 0, 0, ssfmax, anorm, lendsv-lsv, 1, e[lsv:], 1)
		case up:
			impl.Dlascl(General, 0, 0, ssfmin, anorm, lendsv-lsv+1, 1, d[lsv:], 1)
			impl.Dlascl(General, 0, 0, ssfmin, anorm, lendsv-lsv, 1, e[lsv:], 1)
		}
