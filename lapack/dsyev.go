...
// portion specified by uplo. If jobz == ComputeEV a contains the
...
func (impl Implementation) Dsyev(jobz EVJob, uplo blas.Uplo, n int, a []float64, lda int, w, work []float64, lwork int) (ok bool) {
	checkMatrix(n, n, a, lda)
	upper := uplo == blas.Upper
	wantz := jobz == ComputeEV
	var opts string
	if upper {
		opts = "U"
	} else {
	...
	anrm := impl.Dlansy(MaxAbs, uplo, n, a, lda, work)
	scaled := false
	var sigma float64
	if anrm > 0 && anrm < rmin {
		scaled = true
	...
	if !wantz {
		...
		ok = impl.Dsteqr(EVComp(jobz), n, w, work[inde:], a, lda, work[indtau:])
	}
	if !ok {
		return false
	}
