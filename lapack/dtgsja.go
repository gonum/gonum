...
// is as follows
//  jobU == GSVDU        Compute orthogonal matrix U
//  jobU == GSVDUnit     Use unit-initialized matrix
//  jobU == GSVDNone     Do not compute orthogonal matrix.
// The behavior is the same for jobV and jobQ with the exception that instead of
// GSVDU these accept GSVDV and GSVDQ respectively.
// The matrices U, V and Q must be m×m, p×p and n×n respectively unless the
// relevant job parameter is GSVDNone.
//
...
func (impl Implementation) Dtgsja(jobU, jobV, jobQ GSVDJob, m, p, n, k, l int, a []float64, lda int, b []float64, ldb int, tola, tolb float64, alpha, beta, u []float64, ldu int, v []float64, ldv int, q []float64, ldq int, work []float64) (cycles int, ok bool) {
	const maxit = 40
	...
	initu := jobU == GSVDUnit
	wantu := initu || jobU == GSVDU
	if !initu && !wantu && jobU != GSVDNone {
		panic(badGSVDJob + "U")
	}
	if jobU != GSVDNone {
	...
	initv := jobV == GSVDUnit
	wantv := initv || jobV == GSVDV
	if !initv && !wantv && jobV != GSVDNone {
		panic(badGSVDJob + "V")
	}
	if jobV != GSVDNone {
	...
	initq := jobQ == GSVDUnit
	wantq := initq || jobQ == GSVDQ
	if !initq && !wantq && jobQ != GSVDNone {
		panic(badGSVDJob + "Q")
	}
	if jobQ != GSVDNone {
