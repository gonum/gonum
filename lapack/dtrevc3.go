...
//
// If side == RightEV, only right eigenvectors will be computed.
// If side == LeftEV, only left eigenvectors will be computed.
// If side == RightLeftEV, both right and left eigenvectors will be computed.
// For other values of side, Dtrevc3 will panic.
//
// If howmny == AllEV, all right and/or left eigenvectors will be
// computed.
// If howmny == AllEVMulQ, all right and/or left eigenvectors will be
// computed and multiplied from left by the matrices in VR and/or VL.
// If howmny == SelectedEV, right and/or left eigenvectors will be
...
// if howmny == SelectedEV, and it is not referenced otherwise.
...
//
// VL and VR are n×mm matrices. If howmny is AllEV or
// AllEVMulQ, mm must be at least n. If howmny ==
// SelectedEV, mm must be large enough to store the selected
...
//
// On entry, if howmny == AllEVMulQ, it is assumed that VL (if side
// is LeftEV or RightLeftEV) contains an n×n matrix QL,
// and that VR (if side is LeftEV or RightLeftEV) contains
// an n×n matrix QR. QL and QR are typically the orthogonal matrix Q of Schur
// vectors returned by Dhseqr.
//
// On return, if side is LeftEV or RightLeftEV,
// VL will contain:
//  if howmny == AllEV,      the matrix Y of left eigenvectors of T,
//  if howmny == AllEVMulQ,  the matrix Q*Y,
//  if howmny == SelectedEV, the left eigenvectors of T specified by
//                                  selected, stored consecutively in the
//                                  columns of VL, in the same order as their
//                                  eigenvalues.
// VL is not referenced if side == RightEV.
//
// On return, if side is RightEV or RightLeftEV,
// VR will contain:
//  if howmny == AllEV,      the matrix X of right eigenvectors of T,
//  if howmny == AllEVMulQ,  the matrix Q*X,
//  if howmny == SelectedEV, the left eigenvectors of T specified by
//                                  selected, stored consecutively in the
//                                  columns of VR, in the same order as their
//                                  eigenvalues.
// VR is not referenced if side == LeftEV.
//
...
func (impl Implementation) Dtrevc3(side EVSide, howmny HowMany, selected []bool, n int, t []float64, ldt int, vl []float64, ldvl int, vr []float64, ldvr int, mm int, work []float64, lwork int) (m int) {
	switch side {
	default:
		panic(badSide)
	case RightEV, LeftEV, RightLeftEV:
	}
	switch howmny {
	default:
		panic(badHowMany)
	case AllEV, AllEVMulQ, SelectedEV:
	}
	switch {
	case n < 0:
		panic(nLT0)
	...
	if lwork != -1 {
		...
		if (side == RightEV || side == RightLeftEV) && m > 0 {
			checkMatrix(n, m, vr, ldvr)
		}
		if (side == LeftEV || side == RightLeftEV) && m > 0 {
	...
	if howmny == AllEVMulQ && lwork >= n+2*n*nbmin {
...
leftev:
	...
	for ki := 0; ki < n; ki++ {
		...
		if howmny == SelectedEV && !selected[ki] {
			continue
		}
