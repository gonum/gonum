...
func (impl Implementation) Dtgsy2(trans blas.Transpose, ijob lapack.MaximizeNormXJob, m, n int, a []float64, lda int, b []float64, ldb int, c []float64, ldc int, d []float64, ldd int, e []float64, lde int, f []float64, ldf int, rdsum, rdscal float64, iwork []int) (scale, scalout, sumout float64, pq, info int) {
	switch {
			...
			for i := p; i >= 0; i-- {
				...
				if mb == 1 && nb == 1 {
					...
					if ijob == 0 {
						...
						rdsum, rdscal = impl.Dlatdf(ijob, zdim, z, ldz, rhs,
							rdsum, rdscal, ipiv[:zdim], jpiv[:zdim])
					}
					...
					// Substitute R(i, j) and L(i, j) into remaining equation.
					if i > 0 {
						bi.Dgemm(blas.NoTrans, blas.Trans, is, nb, mb, -1,
					...
					if j < q {
						k = mb * nb
						bi.Dgemm(blas.Trans, blas.NoTrans, mb, n-je-1, nb, 1,
