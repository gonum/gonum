// Copyright ©2026 The Gonum Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package testlapack

import (
	"fmt"
	"math/rand/v2"
	"testing"

	"gonum.org/v1/gonum/blas"
)

type Zlacpyer interface {
	Zlacpy(uplo blas.Uplo, m, n int, a []complex128, lda int, b []complex128, ldb int)
}

func ZlacpyTest(t *testing.T, impl Zlacpyer) {
	rnd := rand.New(rand.NewPCG(1, 1))
	for _, uplo := range []blas.Uplo{blas.Upper, blas.Lower, blas.All} {
		for _, test := range []struct {
			m, n, lda, ldb int
		}{
			{3, 5, 0, 0},
			{5, 5, 0, 0},
			{7, 5, 0, 0},
			{3, 5, 10, 12},
			{5, 5, 10, 12},
			{7, 5, 10, 12},
		} {
			m := test.m
			n := test.n
			lda := test.lda
			if lda == 0 {
				lda = n
			}
			ldb := test.ldb
			if ldb == 0 {
				ldb = n
			}
			a := make([]complex128, m*lda)
			for i := range a {
				a[i] = complex(rnd.NormFloat64(), rnd.NormFloat64())
			}
			b := make([]complex128, m*ldb)
			for i := range b {
				b[i] = complex(rnd.NormFloat64(), rnd.NormFloat64())
			}
			impl.Zlacpy(uplo, m, n, a, lda, b, ldb)
			equal := true
			switch uplo {
			case blas.Upper:
				for i := 0; i < m; i++ {
					for j := i; j < n; j++ {
						if b[i*ldb+j] != a[i*lda+j] {
							equal = false
							goto DoneCheck
						}
					}
				}
			case blas.Lower:
				for i := 0; i < m; i++ {
					for j := 0; j < min(i, n); j++ {
						if b[i*ldb+j] != a[i*lda+j] {
							equal = false
							goto DoneCheck
						}
					}
				}
			case blas.All:
				for i := 0; i < m; i++ {
					for j := 0; j < n; j++ {
						if b[i*ldb+j] != a[i*lda+j] {
							equal = false
							goto DoneCheck
						}
					}
				}
			}
		DoneCheck:
			if !equal {
				t.Errorf("%s", fmt.Sprintf("Zlacpy: not equal. uplo=%v m=%v n=%v", uplo, m, n))
			}
		}
	}
}
