			...
			if ul == blas.Upper {
				for i := m - 1; i >= 0; i-- {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						k := ka + i + 1
						if va != 0 {
							asm.DaxpyUnitary(-va, b[k*ldb:k*ldb+n], btmp)
						}
					}
					if nonUnit {
						tmp := 1 / a[i*lda+i]
		...
		if ul == blas.Upper {
			for k := 0; k < m; k++ {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					i := ia + k + 1
					if va != 0 {
						asm.DaxpyUnitary(-va, btmpk, b[i*ldb:i*ldb+n])
					}
				}
				if alpha != 1 {
					for j := 0; j < n; j++ {
				...
				for k, vb := range btmp {
					if vb != 0 {
						if btmp[k] != 0 {
							...
							asm.DaxpyUnitary(-btmp[k], a[k*lda+k+1:k*lda+n], btmp[k+1:n])
						}
					}
				}
			}
		...
		for i := 0; i < m; i++ {
			...
			for k := n - 1; k >= 0; k-- {
				if btmp[k] != 0 {
					...
					asm.DaxpyUnitary(-btmp[k], a[k*lda:k*lda+k], btmp)
				}
			}
		}
		return
	...
	if s == blas.Left {
		for i := 0; i < m; i++ {
			...
			for k := 0; k < i; k++ {
				...
				atmp *= alpha
				asm.DaxpyUnitary(atmp, b[k*ldb:k*ldb+n], c[i*ldc:i*ldc+n])
			}
			for k := i + 1; k < m; k++ {
				var atmp float64
				if isUpper {
			...
			if ul == blas.Upper {
				for i := 0; i < m; i++ {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						...
						tmp := alpha * va
						if tmp != 0 {
							asm.DaxpyUnitary(tmp, b[k*ldb:k*ldb+n], btmp)
						}
					}
				}
				return
		...
		if ul == blas.Upper {
			for k := m - 1; k >= 0; k-- {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					tmp := alpha * va
					if tmp != 0 {
						asm.DaxpyUnitary(tmp, btmpk, btmp)
					}
				}
				tmp := alpha
				if nonUnit {
		...
		for i := 0; i < m; i++ {
			...
			for k := 0; k < n; k++ {
				...
				if tmp != 0 {
					...
					asm.DaxpyUnitary(tmp, a[k*lda:k*lda+k], btmp)
				}
			}
		}
		return
