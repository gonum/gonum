...
func Parametric(c []float64, A mat.Matrix, b []float64, tol float64, initialBasic []int, rnd *rand.Rand) (optF float64, optX []float64, err error) {
    ans, x, _, _, err := parametric(initialBasic, c, A, b, nil, nil, tol, false, rnd)
	return ans, x, err
}
...
//  minimize ‖θ‖_1
//  s.t. 		‖∇L(θ)‖_∞ ≤ λ.
// Either fomulation can both be expressed as linear a program when L(θ)
...
func ParametricWithTrace(c []float64, A mat.Matrix, b, cbar, bbar []float64, tol float64, initialBasic []int, rnd *rand.Rand) (optTr *Trace, err error) {
    _, _, _, tr, err := parametric(initialBasic, c, A, b, cbar, bbar, tol, true, rnd)
	return tr, err
}

func parametric(initialBasic []int, c []float64, A mat.Matrix, b,
        cbar, bbar []float64, tol float64, trace bool, rnd *rand.Rand) (float64, []float64, []int, *Trace, error) {
...
func verifyPerturbations(c, b, cbar, bbar []float64) {
	if cbar != nil && len(cbar) != len(c) {
		panic("lp: cbar vector incorrect length")
	}
	if bbar != nil && len(bbar) != len(b) {
