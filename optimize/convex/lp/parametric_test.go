...
func TestParametric(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// solve and check
		opt, x, err := Parametric(test.c, test.A, test.b, test.tol, nil, rnd)
		if err != nil {
...
func testParametric(t *testing.T, initialBasic []int, c []float64, a mat.Matrix, b []float64, lambdaTol float64, rnd *rand.Rand) error {
	primalOpt, primalX, _, _, errPrimal := parametric(initialBasic, c, a, b, nil, nil, lambdaTol, false, rnd)
	if errPrimal == nil {
	...
	dualOpt, dualX, _, _, errDual := parametric(nil, cNew, aNew, bNew, nil, nil, lambdaTol, false, rnd)
	if errDual == nil {
