...
func TestNoncentralTRand(t *testing.T) {
	...
	tests := []struct {
		...
		{dist: NoncentralT{Nu: 58, Ncp: 1.9364916731037085, Src: rsrc}, tol: 1e-2},
		{dist: NoncentralT{Nu: 198, Ncp: 7.0710678118654755, Src: rsrc}, tol: 1e-2},
	}
...
func TestNoncentralTMoments(t *testing.T) {
	...
	tests := []struct {
		...
		{dist: NoncentralT{Nu: 20, Ncp: 23}, mean: 23.909902487370097, variance: 17.205451933342147, tol: 1e-13},
	}
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			mean := test.dist.Mean()
			if !scalar.EqualWithinRel(mean, test.mean, test.tol) {
				t.Errorf("{%f, %f}.Mean: got %f want %f", test.dist.Nu, test.dist.Ncp, mean, test.mean)
			}
			variance := test.dist.Variance()
			if !scalar.EqualWithinRel(variance, test.variance, test.tol) {
				t.Errorf("{%f, %f}.Variance: got %f want %f", test.dist.Nu, test.dist.Ncp, variance, test.variance)
			}
		})
	}
}
...
func TestNoncentralTProb(t *testing.T) {
	...
	tests := []struct {
		...
		{dist: NoncentralT{Nu: 10, Ncp: 0}, x: 1.8, p: 0.08311638965387959, tol: 5e-12},
		{dist: NoncentralT{Nu: 10, Ncp: 0.0001}, x: 1.8, p: 0.0831297211093226, tol: 5e-12},
		{dist: NoncentralT{Nu: 10, Ncp: 1}, x: 1.8, p: 0.2665039326310101, tol: 5e-12},
		{dist: NoncentralT{Nu: 10, Ncp: -0.0001}, x: 1.8, p: 0.08310305960463463, tol: 5e-12},
		{dist: NoncentralT{Nu: 10, Ncp: -1}, x: 1.8, p: 0.01074629362995775, tol: 5e-12},

		// Based on https://github.com/boostorg/math/blob/develop/test/test_nc_t.hpp
		{dist: NoncentralT{Nu: 8, Ncp: 12}, x: 12, p: 1.23532971542589493e-1, tol: 5e-12},
		{dist: NoncentralT{Nu: 126, Ncp: -2}, x: -4, p: 5.79793228936581470e-2, tol: 2e-11},
		{dist: NoncentralT{Nu: 126, Ncp: 2}, x: 4, p: 5.79793228936581470e-2, tol: 2e-11},
		{dist: NoncentralT{Nu: 126, Ncp: 2}, x: 0, p: 5.38839489063995713e-2, tol: 5e-12},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: 0, p: 9.94670846108541165e-57, tol: 5e-12},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: 0x1p-1022, p: 9.94670846108541165e-57, tol: 5e-12},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: -0x1p-1022, p: 9.94670846108541165e-57, tol: 5e-12},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: -0.125, p: 1.40958893993909266e-57, tol: 5e-12, abs: 1e-16},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: -1e-16, p: 9.94670846108539523e-57, tol: 5e-12},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: 1e-16, p: 9.94670846108542807e-57, tol: 5e-12},
		{dist: NoncentralT{Nu: 8, Ncp: 16}, x: 0.125, p: 8.78741270305645722e-56, tol: 5e-4},

		// Custom tests, wanted values from the R language version 4.4.2.
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: -0.3930852906078905, p: 0.02631178912036441, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 0.6553966734339551, p: 0.1756830868490319, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 1.498837237776003, p: 0.3591385592167584, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 1.944945574355751, p: 0.3908781127965009, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 2.369093423208461, p: 0.3534073785987947, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 3.290148485967974, p: 0.1615374126723982, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 4.469416864177883, p: 0.02264358202648499, tol: 5e-12},
		{dist: NoncentralT{Nu: 58, Ncp: 1.936492}, x: 0, p: 0.0609166161931202, tol: 5e-12},
	}
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			p := test.dist.Prob(test.x)
			if !scalar.EqualWithinAbsOrRel(p, test.p, test.abs, test.tol) {
				t.Errorf("{%f, %f}.Prob(%f): got %g want %g", test.dist.Nu, test.dist.Ncp, test.x, p, test.p)
			}
		})
	}
}
...
func TestNoncentralTCDF(t *testing.T) {
	...
	tests := []struct {
		...
		{dist: NoncentralT{Nu: 10, Ncp: 38.5}, x: 40, cdf: 0.5073909173564686, tol: 5e-3},
	}
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
