...
/*
	xCopy = sliceCopy(test.x)
	aFlatDense := flatten(test.a)
	blasser.Dtrsv(test.ul, test.tA, test.d, test.n, aFlatDense, len(test.a[0]), xCopy, test.incX)
	if !dSliceTolEqual(test.ans, xCopy) {
		t.Errorf("Case %v: mismatch with Dense Want %v, got %v", i, test.ans, xCopy)
	}
*/
